...
def verify_heartbeat_auth(
    ...
    """Raise HTTPException(401) if the heartbeat fails verification. No-op when valid."""
    ...
    if timestamp is None or signature is None:
        if mode == "enforce":
            raise HTTPException(status.HTTP_401_UNAUTHORIZED, "missing heartbeat signature")
    ...
    if abs(time.time() - ts) > MAX_SKEW_SECONDS:
        raise HTTPException(status.HTTP_401_UNAUTHORIZED, "stale timestamp (possible replay)")
